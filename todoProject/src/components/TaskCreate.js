...
setTaskDesc('');
    ...
    return (  
    <div>
        {taskformUpdate ?  (<div className="task-upd">
        <h3>Lütfen Yapılacakları Düzenleyiniz!</h3>
        ...
        <h3>Lütfen Yapılacakları Ekleyiniz!</h3>
        <form className="task-form">
            <label className="task-label">Başlık</label>
           <input value={title} onChange={handleChange} className="task-input"/>
           <label className="task-label">Yapılacakları Giriniz!</label>
           <textarea value={taskDesc} onChange={handleTaskChange} className="task-input" rows={5}/>
           <button className="task-button" onClick={handleSubmit}>Kaydet</button>
        </form>
   </div>)
